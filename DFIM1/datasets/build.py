...
__factory = {'RSICD': RSICD, 'RSITMD': RSITMD}
...
def build_dataloader(args, tranforms=None):
    ...
    Args:
        ...
        tranforms (callable, optional): 图像转换函数。默认为None。

    Returns:
        tuple: 包含训练集数据加载器、验证集图像数据加载器、验证集文本数据加载器以及类别数量的元组。
            - train_loader (torch.utils.data.DataLoader): 训练集数据加载器。
            - val_img_loader (torch.utils.data.DataLoader): 验证集图像数据加载器。
            - val_txt_loader (torch.utils.data.DataLoader): 验证集文本数据加载器。
            - num_classes (int): 类别数量。

    Raises:
        ValueError: 如果采样器类型不受支持，将引发此异常。
    """
